
import React from 'react';
import Heading from './Heading';
import './Projects.css';
import Navbar from '../pages/Navbar';

function Menu() {
    return (
        <div>
            <Navbar />
            <Heading />
            <section className="menu" id="menu">
                <div>
                    <div className="grid-container">

                        <div className="box" style={{ marginTop: '20px' }}>
                            <img src="/img/portfolio.png" alt="" className="left" />
                            <div className="right">
                                <h3>Portfolio-Website</h3>
                                <p>Portfolio-Website Using React.js</p>
                                <a className="price" href="https://github.com/tahaanwar2/taha-portfolio.git" target="_blank" rel="noopener noreferrer">
                                    Source Code
                                </a></div>
                        </div>



                        <div className="box" style={{ marginTop: '20px' }}>
                            <img src="/img/contact-app.png" alt="" className="left" />
                            <div className="right">
                                <h3>Firebase-Contact-App</h3>
                                <p>Figma Design Using React</p>
                                <a className="price" href="https://github.com/tahaanwar2/Firebase-Contact-App.git" target="_blank" rel="noopener noreferrer">
                                    Source Code
                                </a></div>
                        </div>

                        <div className="box" style={{ marginTop: '20px' }}>
                            <img src="/img/rest website.png" alt="" className="left" />
                            <div className="right">
                                <h3>Resturent Website </h3>
                                <p>UI using React.js</p>
                                <a className="price" href="link-to-github-repo" target="_blank" rel="noopener noreferrer">
                                    Source Code
                                </a></div>
                        </div>

                        <div className="box" style={{ marginTop: '20px' }}>
                            <img src="/img/login.png" alt="" className="left" />
                            <div className="right">
                                <h3>Login/Sign Up </h3>
                                <p>Firebase Authentication using React.js</p>
                                <a className="price" href="https://github.com/tahaanwar2/Firebase-Login-Signup.git" target="_blank" rel="noopener noreferrer">
                                    Source Code
                                </a></div>
                        </div>


                        <div className="box" style={{ marginTop: '20px' }}>
                            <img src="/img/foody.png" alt="" className="left" />
                            <div className="right">
                                <h3>Foody-Zone</h3>
                                <p>Responsive Foody Design Using React.</p>
                                <a className="price" href="link-to-github-repo" target="_blank" rel="noopener noreferrer">
                                    Source Code
                                </a></div>
                        </div>
                        <div className="box" style={{ marginTop: '20px' }}>
                            <img src="/img/food order.png" alt="" className="left" />
                            <div className="right">
                                <h3>Food Order</h3>
                                <p>Food buying ordering using html, css, js.</p>
                                <a className="price" href="link-to-github-repo" target="_blank" rel="noopener noreferrer">
                                    Source Code
                                </a></div>
                        </div>





                        <div className="box" style={{ marginTop: '20px' }}>
                            <img src="/img/shoes.png" alt="" className="left" />
                            <div className="right">
                                <h3>Shoe Brand Page</h3>
                                <p>Figma Design Using React</p>
                                <a className="price" href="link-to-github-repo" target="_blank" rel="noopener noreferrer">
                                    Source Code
                                </a>      </div>
                        </div>

                        <div className="box" style={{ marginTop: '20px' }}>
                            <img src="/img/contact-page.png" alt="" className="left" />
                            <div className="right">
                                <h3>Contact-Page</h3>
                                <p>.</p>
                                <a className="price" href="link-to-github-repo" target="_blank" rel="noopener noreferrer">
                                    Source Code
                                </a></div>
                        </div>


                        <div className="box" style={{ marginTop: '20px' }}>
                            <img src="/img/ecom.png" alt="" className="left" />
                            <div className="right">
                                <h3>E-commerce Ui/Ux</h3>
                                <p>Using html,css,js.</p>
                                <a className="price" href="link-to-github-repo" target="_blank" rel="noopener noreferrer">
                                    Source Code
                                </a></div>
                        </div>

                        <div className="box" style={{ marginTop: '20px' }}>
                            <img src="/img/hotel.png" alt="" className="left" />
                            <div className="right">
                                <h3>Hotel Management System </h3>
                                <p>Hotel Management System using Python</p>
                                <a className="price" href="https://github.com/tahaanwar2/Hotel-management.git" target="_blank" rel="noopener noreferrer">
                                    Source Code
                                </a></div>
                        </div>



                        <div className="box" style={{ marginTop: '20px' }}>
                            <img src="/img/conta.png" alt="" className="left" />
                            <div className="right">
                                <h3>Exercise-App</h3>
                                <p>Fitness exercise app.</p>
                                <a className="price" href="https://github.com/tahaanwar2/fyp-exercise-app.git" target="_blank" rel="noopener noreferrer">
                                    Source Code
                                </a></div>
                        </div>



                        <div className="box" style={{ marginTop: '20px' }}>
                            <img src="/img/eye follow.png" alt="" className="left" />
                            <div className="right">
                                <h3>Eye Follow</h3>
                                <p> Eye Follow mouse cursor using html, css, js.</p>
                                <a className="price" href="link-to-github-repo" target="_blank" rel="noopener noreferrer">
                                    Source Code
                                </a></div>
                        </div>
                        <div className="box" style={{ marginTop: '20px' }}>

                            <img src="/img/palind.png" alt="" className="left" />
                            <div className="right">
                                <h3>Palindrome Word Checker</h3>
                                <p>Palindrome Word Checker using html, css, js.</p>
                                <a className="price" href="link-to-github-repo" target="_blank" rel="noopener noreferrer">
                                    Source Code
                                </a></div>
                        </div>

                        <div className="box" style={{ marginTop: '20px' }}>

                            <img src="/img/kij.png" alt="" className="left" />
                            <div className="right">
                                <h1>See All</h1>
                                <p>*****.</p>
                                <a className="price" href="https://github.com/tahaanwar2" target="_blank" rel="noopener noreferrer">
                                    Github Link
                                </a></div>
                        </div>



                    </div>

                </div>
            </section>
        </div>
    );
}

export default Menu;
